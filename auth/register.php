  ...
  if(isset($_POST['submit'])) {
    ...
    if(empty($_POST['username']) OR empty($_POST['email']) OR empty($_POST['password']) OR empty($_POST['re-password'])) {
      ...
      if($password == $repassword) {
        ...
        if(strlen($email) > 50 OR strlen($username) > 50 ) {
      ...
      <div class="col-md-12 mb-5">
        <form action="register.php" class="p-4 border rounded" method="POST">
          ...
          <div class="row form-group mb-4">
            <div class="col-md-12 mb-3 mb-md-0">
              <label class="text-black" for="re-password">Re-Type Password</label>
              <input type="password" id="re-password" class="form-control" placeholder="Re-type Password" name="re-password">
            </div>
          </div>
